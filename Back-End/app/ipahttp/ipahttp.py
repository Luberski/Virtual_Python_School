...
class ipa(object):
    ...
    def login(self, user, password):
        ...
        rv = self.session.post(
            ipaurl, headers=header, data=login, verify=self.sslverify
        )
    ...
    def checkAvailability(self):
        retVal = requests.get(
            "https://{0}/ipa/".format(self.server), verify=self.sslverify, timeout=5
        )
    ...
    def makeReq(self, pdict):
        ...
        request = self.session.post(
            session_url, headers=header, data=json.dumps(data), verify=self.sslverify
        )
